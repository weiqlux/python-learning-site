...
本文档说明如何将喜马拉雅音频内容集成到 `python-learning-site` 项目中，用于学习音频资料输出和思维导图生成。
...
class XimalayaClient:
    ...
    def _get_access_token(self) -> Optional[str]:
        ...
        url = f"{self.base_url}/oauth2/v2/access_token"
    ...
    def get_recommend_albums(self, category_id: int = 0, count: int = 20) -> Dict:
        """获取推荐专辑"""
        url = f"{self.base_url}/recommend/albums"
        params = self._get_common_params()
        params.update({
...
@app.route('/api/ximalaya/track/<track_id>/playurl')
def ximalaya_track_playurl(track_id):
    """获取声音播放地址"""
    if not ximalaya_client:
        return jsonify({'error': '喜马拉雅 API 未配置'}), 500
    
    result = ximalaya_client.get_track_playurl(track_id)
    return jsonify(result)
```
...
<body>
    ...
    <div id="trackModal" style="display:none; position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.5);">
        <div style="background:white; margin:50px auto; padding:20px; max-width:800px; border-radius:8px;">
            ...
            <button onclick="closeModal()" style="margin-top:20px; padding:10px 20px;">关闭</button>
        </div>
    </div>
    ...
    <script>
        async function searchAlbums() {
            ...
            if (data.albums) {
                data.albums.forEach(album => {
                    ...
                    `;
                    card.onclick = () => showAlbumTracks(album.id);
        ...
        async function showAlbumTracks(albumId) {
            ...
            document.getElementById('albumTitle').textContent = data.album_title || '专辑';
            ...
            if (data.tracks) {
                data.tracks.forEach(track => {
                    ...
                    li.innerHTML = `
                        ...
                        <button class="play-btn" onclick="playTrack('${track.id}')">播放</button>
                    `;
        ...
        async function playTrack(trackId) {
            const response = await fetch(`/api/ximalaya/track/${trackId}/playurl`);
            const data = await response.json();
            
            if (data.play_url) {
                const audio = new Audio(data.play_url);
                audio.play();
            } else {
                alert('获取播放地址失败');
            }
